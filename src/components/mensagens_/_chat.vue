    ...
    <template #header>
      <div class="chat-header flex flex-col relative w-full">
        ...
        <div class="absolute right-0 top-0 flex items-center">
          <Badge v-if="chatDetails.seen" value="Salvo" severity="success" />
      ...
      <div class="chat-panel" style="height: calc(100vh - 230px)">
        ...
        <div class="message-input-container">
          <div class="message-input-wrapper">
            ...
            />
            <Button icon="pi pi-send" @click="sendResponse" />
          </div>
        </div>
      </div>
            ...
            <div class="info-panel">
              <div class="info-section">
                ...
                <div class="info-row" v-if="chatDetails.order_id">
                  ...
                  <span
                    v-if="
                      chatDetails.order_data &&
                      chatDetails.order_data.marketplace === 'leroy'
                    "
                    class="info-value"
                  >
                    <a
                      :href="`https://adeo-marketplace.mirakl.net/mmp/shop/order/${chatDetails.order_id}`"
                      target="_blank"
                      rel="noopener noreferrer"
                      >{{ chatDetails.order_id }}</a
                    >
                  </span>
                  <span
                    v-else-if="
                      chatDetails.order_data &&
                      chatDetails.order_data.marketplace === 'worten'
                    "
                    class="info-value"
                  >
                    <a
                      :href="`https://marketplace.worten.pt/mmp/shop/order/${chatDetails.order_id}`"
                      target="_blank"
                      rel="noopener noreferrer"
                      >{{ chatDetails.order_id }}</a
                    >
                  ...
                  <span v-else class="info-value">
                    {{ chatDetails.order_id }}
                  </span>
                </div>
                <!-- Adicionando o campo vevor_id -->
                <div
                  class="info-row"
                  v-if="
                    chatDetails.order_data && chatDetails.order_data.vevor_id
                  "
                >
                  <span class="info-label">Vevor ID:</span>
                  <span class="info-value">{{
                    chatDetails.order_data.vevor_id
                  }}</span>
                </div>
                <div class="info-row" v-if="chatDetails.status">
                  ...
                  <span class="info-value">
                    <Badge
                      :value="chatDetails.status"
                      :severity="getStatusSeverity(chatDetails.status)"
                    />
                  </span>
                </div>
          ...
          <TabPanel value="1">
            <div class="info-panel" v-if="chatDetails.order_data">
              <div class="info-section">
                ...
                <div
                  class="info-row"
                  v-if="chatDetails.order_data.product_title"
                >
                  <span class="info-label">Produto:</span>
                  <span class="info-value">{{
                    chatDetails.order_data.product_title
                  }}</span>
                </div>
                <div class="info-row" v-if="chatDetails.order_data.price">
                  <span class="info-label">Preço:</span>
                  <span class="info-value">{{
                    formatPrice(chatDetails.order_data.price)
                  }}</span>
                </div>
                <div class="info-row" v-if="chatDetails.order_data.quantity">
                  <span class="info-label">Quantidade:</span>
                  <span class="info-value">{{
                    chatDetails.order_data.quantity
                  }}</span>
                </div>
                <div class="info-row" v-if="chatDetails.order_data.stock">
                  <span class="info-label">Estoque:</span>
                  <span class="info-value">{{
                    chatDetails.order_data.stock
                  }}</span>
                </div>
                <div
                  class="info-row"
                  v-if="chatDetails.order_data.last_updated_date"
                >
                  <span class="info-label">Última Atualização:</span>
                  <span class="info-value">{{
                    formatDate(chatDetails.order_data.last_updated_date)
                  }}</span>
                </div>
                <div class="info-row" v-if="chatDetails.order_data.link">
                  ...
                  <span class="info-value">
                    <a
                      :href="chatDetails.order_data.link"
                      target="_blank"
                      class="product-link"
                    >
            ...
            <p v-else class="no-data-message">
              Não há informações disponíveis sobre o produto
            </p>
          </TabPanel>
          ...
          <TabPanel value="2">
            <div class="info-panel" v-if="chatDetails.order_data">
              <div class="info-section">
                ...
                <div
                  class="info-row"
                  v-if="chatDetails.order_data.shipping_company"
                >
                  <span class="info-label">Transportadora:</span>
                  <span class="info-value">{{
                    chatDetails.order_data.shipping_company
                  }}</span>
                </div>
                <div
                  class="info-row"
                  v-if="chatDetails.order_data.shipping_carrier_code"
                >
                  <span class="info-label">Código:</span>
                  <span class="info-value">{{
                    chatDetails.order_data.shipping_carrier_code
                  }}</span>
                </div>
                <div
                  class="info-row"
                  v-if="chatDetails.order_data.info_logistic"
                >
                  <span class="info-label">Info. Logística:</span>
                  <span class="info-value">{{
                    chatDetails.order_data.info_logistic
                  }}</span>
                </div>
                <div
                  class="info-row"
                  v-if="
                    chatDetails.order_data.shipping_link ||
                    chatDetails.shipping_link
                  "
                >
              ...
              <div class="info-section">
                ...
                <div class="info-row" v-if="chatDetails.order_data.marketplace">
                  ...
                  <span class="info-value">{{
                    chatDetails.order_data.marketplace
                  }}</span>
                </div>
                <div class="info-row" v-if="chatDetails.order_data.state">
                  <span class="info-label">Estado:</span>
                  <span class="info-value">{{
                    chatDetails.order_data.state
                  }}</span>
                </div>
              </div>
            ...
            <p v-else class="no-data-message">
              Não há informações de rastreio disponíveis
            </p>
          </TabPanel>
...
// Adicionar variável notas que falta
const notas = ref([]);

// Adicionar um array local para rastrear mensagens ignoradas
const ignoredMessageIds = ref([]);

// Adicionar uma variável para rastrear se a mensagem atual veio da edição da IA
const messageFromAIEdit = ref(false);

// Função para criar um objeto chatDetails vazio com estrutura válida
const getDummyValue = () => {
  return {
    thread_id: null,
    status: "pendente",
...
const {
  ...
  reversedMessages,
  canSendMessage,
  fetchChatDetails: fetchChatDetailsFromComposable,
...
// Garantir que chatDetails tenha um valor padrão com estrutura adequada
if (!chatDetails.value) {
  chatDetails.value = getDummyValue();
}

// Função auxiliar para obter um identificador único para uma mensagem
const getMessageId = (message) => {
  return message.id || message._id || `msg_${message.date_created}`;
};

// Adicionar uma propriedade computada para filtrar as mensagens ignoradas
const filteredMessages = computed(() => {
  if (!reversedMessages.value) return [];
  return reversedMessages.value.filter((msg) => {
    const msgId = getMessageId(msg);
    return !ignoredMessageIds.value.includes(msgId);
  });
});

// Métodos auxiliares
const messageClasses = (message) => {
  return isOutgoingMessage(message) ? "outgoing-bubble" : "incoming-bubble";
};
...
const openOptionsMenu = (event) => {
  ...
  // Verificar se chatDetails.value existe antes de acessar propriedades
  if (!chatDetails.value) return;
...
const updateChatStatus = async (newStatus) => {
  try {
    const response = await ChatService.setStatus(threadId.value, {
      status: newStatus,
    });
    if (response) {
...
// Função para carregar os detalhes do chat
const loadChatDetails = async () => {
  ...
  // Limpar a lista de mensagens ignoradas ao carregar um novo chat
  ignoredMessageIds.value = [];
  ...
  try {
    ...
    // Garantir que order_data exista, mesmo que vazio
    if (!response.order_data) {
      response.order_data = {};
    }

    chatDetails.value = response;
    // Não usamos mais a variável messages local
...
// Funções para tratar as ações dos botões das mensagens da IA
const handleIgnoreMessage = (messageId) => {
  // Adicionar o ID da mensagem à lista de ignorados
  if (messageId) {
    ignoredMessageIds.value.push(messageId);

    toast.add({
      severity: "info",
      summary: "Mensagem ignorada",
      detail: "A mensagem foi ocultada",
      life: 2000,
    });
  }
};
...
const handleEditMessage = (messageData) => {
  ...
  // Coloca o texto da mensagem no input
  newMessage.value = messageData.text || "";
  
  // Marca que esta mensagem veio da edição da IA
  messageFromAIEdit.value = true;

  // Remove a mensagem original adicionando à lista de ignorados
  ignoredMessageIds.value.push(messageData.id);

  // Foca no input para edição
  setTimeout(() => {
    const inputElement = document.querySelector(".message-input-wrapper input");
    if (inputElement) inputElement.focus();
  }, 100);

  // Exibir aviso rápido sobre manter a mensagem passiva
  toast.add({
    severity: "info",
    summary: "Dica de edição",
    detail: "Tome cuidado ao editar esta mensagem, tente mantê-la passiva, da forma como gostaria que a IA respondesse",
    life: 4000,
  });
};
...
const handleSendMessage = async (messageData) => {
  ...
  // Quando vem dos botões, SEMPRE adiciona o "+" no final
  let send_name = messageData.sender || "";
  if (send_name === "IA") {
    send_name = "admin+";
  } else if (send_name === "IA+") {
    send_name = "admin+";
  } else if (send_name === "") {
    send_name = "admin+";
  } else if (send_name === "+") {
    ...
    // Garante que tenha + no final para qualquer outro nome
    if (!send_name.endsWith("+")) send_name += "+";
  }

  // Envia a mensagem como se fosse do usuário com o + no final do send_name
  try {
...
// Modificar sendResponse para verificar se a mensagem atual veio da edição da IA
const sendResponse = async () => {
  ...
  // Verificar se esta mensagem veio da edição da IA
  let send_name;
  if (messageFromAIEdit.value) {
    // Se veio da edição da IA, adicionar o +
    send_name = Profile.data.nome ? `${Profile.data.nome}+` : "admin+";
    // Resetar o flag após enviar
    messageFromAIEdit.value = false;
  } else {
    // Caso contrário, enviar sem o +
    send_name = Profile.data.nome || null;
  }

  try {
    const response = await ChatService.respondToChat(threadId.value, {
      response: newMessage.value,
      send_name: send_name,
    });
...
// Adicionar um listener para monitorar mudanças na mensagem
watch(newMessage, (newValue, oldValue) => {
  // Se o usuário modificar significativamente a mensagem que veio da IA
  if (messageFromAIEdit.value && newValue && oldValue && 
      newValue.length > 0 && 
      Math.abs(newValue.length - oldValue.length) > 20) {
    
    // Exibir um aviso sobre modificação extensa do texto
    toast.add({
      severity: "warn",
      summary: "Atenção",
      detail: "Notamos que você modificou muito o texto da IA, se lembre de não escrever mensagens ativas.",
      life: 6000,
      closable: true,
    });
    
    // Mantém o flag para adicionar + mesmo com modificação extensa
    // messageFromAIEdit.value = false; (remover ou comentar esta linha)
  }
});

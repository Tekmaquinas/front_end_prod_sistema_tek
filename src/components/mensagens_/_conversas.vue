    ...
    <template #content>
      ...
      >
        ...
        <Column field="total_messages">
          <template #body="slotProps">
            <!-- Badge vermelho para chats não respondidos (shop_reply_needed = true) -->
            <Badge
        ...
        <Column style="width: 0.1rem; padding: 0px">
          <template #body="{ data }">
            <Tag
              v-if="data.order_data?.has_incident == true"
              v-tooltip.top="{
                value: 'Incidente Aberto',
                pt: { text: '!bg-primary !text-primary-contrast !font-medium' },
              }"
              class="pi pi-exclamation-circle"
              severity="warn"
              style="font-size: small; background-color: transparent"
            />
          </template>
        </Column>
...
const statusOptions = [
  { label: "Por Responder +24h", value: "atencao", severity: "warning" },
  ...
  { label: "Respondido", value: "respondido", severity: "success" },
];
...
const openOptionsMenu = (event, chat) => {
  ...
  // Mostra o menu no local do evento (onde o usuário clicou)
...
const toggleSaveStatus = async (chat) => {
  try {
    // Usar thread_id
    const response = await ChatService.setSeenStatus(chat.thread_id, {
      seen: !chat.seen,
    });

    // Atualizar o item na lista localmente
    if (response) {
      const updatedChat = chats.value.find(
        (c) => c.thread_id === chat.thread_id
      );
      if (updatedChat) {
...
const fetchChats = async () => {
  ...
  try {
    ...
    switch (state.value) {
      case "nao_lidos":
        // Chats não respondidos (cliente foi o último a responder)
        params.notific = "true";
        params.exclude_status = "operador";
        break;
      case "lidos":
        // Chats respondidos (operador foi o último a responder)
        params.notific = "false";
        break;
      case "atencao":
        // Chats por responder +24h
        params.alert = "true";
        break;
      case "urgente":
 ...
 * shop_reply_needed: true = chat NÃO RESPONDIDO (precisa de resposta)
 * shop_reply_needed: false = chat RESPONDIDO (não precisa de resposta)
 */
const rowClass = (data) => {
 ...
 * shop_reply_needed: true = chat NÃO RESPONDIDO (em negrito)
 * shop_reply_needed: false = chat RESPONDIDO (em cor cinza)
 */
const rowStyle = (data) => {
...
const onRowSelect = (event) => {
  ...
  if (chat && chat.thread_id) {
    router.push(
      `/inbox/${chat.marketplace}/${route.params.state}/chat/${chat.thread_id}`
    );
  } else {

<template>
    <Breadcrumb :home="bread_crumb_home" :model="bread_crumb_itens" style="background-color: transparent; padding: 5px 0px 15px;">
        <template #item="{ item, props }">
            <router-link v-if="item.route" v-slot="{ href, navigate }" :to="item.route" custom>
                <a :href="href" v-bind="props.action" @click="navigate">
                    <span :class="[item.icon, 'text-color']" />
                    <span v-if="item?.last" class="text-primary font-semibold">{{ item.label }}</span>
                    <span v-else class="text-surface-500 dark:text-surface-0">{{ item.label }}</span>
                </a>
            </router-link>
            <a v-else :href="item.url" :target="item.target" v-bind="props.action">
                <span class="text-surface-500 dark:text-surface-0">{{ item.label }}</span>
            </a>
        </template>
...
<script setup>
    ...
    const bread_crumb_home = ref({
    icon: 'pi pi-home',
    route: '/'
    });
    const bread_crumb_itens = ref([
        { label: 'Ofertas', route: '/offers/leroy'},
        { label: 'Leroy Merlin', route: '/offers/imports',  last:true}
    ]);

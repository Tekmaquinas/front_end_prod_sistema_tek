  ...
  <div style="margin: auto .5rem; padding: .5rem; background-color: white;  flex: 1; border-radius: 15px;">
    ...
    <DataTable
      ...
      :pt="{
        ...
        column: { bodycell: ({ state }) => (state.loading ? { style: 'opacity: 0.5' } : {}) },
      }"
    >
      <template #header>
        <div class="flex flex-wrap items-center justify-between gap-5" style=" margin-bottom: .2rem;">
          <div class="flex flex-wrap items-center justify-between gap-5">
            <span class="font-bold">Pedidos CARREFOUR</span>
            <div class="flex flex-wrap items-center justify-between gap-5">
...
import { getSeverityIcon, getNameStatus, saveToCache, getFromCache, fetchOrders, updateLink, searching_func } from '../components/utils.mjs';
import SearchBar from '../components/filters_query.vue';

const marketplace = ref('carrefour');
const base_redirect = ref('https://carrefoures-prod.mirakl.net');
...
const statusMaps2 = { 1: 'Script', 2: 'Gestor' };
  ...
  if (updatedData != false) {
    ...
    response_products.forEach((response_product) => {
      const index = products.value.findIndex(
        (product) => product.unique_id_order === response_product.unique_id_order
      );
      if (index !== -1) {
...
const sendchanges = async (selectedProduct) => {
  ...
  const payload = { itens, assigment: selectedAssigment.value['code'] };
...
async function onRowStockUpdate(dataproduct) {
  ...
  try {
    ...
    if (index !== -1) {
      ...
      products.value[index].state_progress = response.state_progress;
      toast.add({ severity: 'success', summary: 'Progresso Atualizado', detail: `O Progresso foi atualizado para ${response.stock}`, group: 'br', life: 3000 });
    }
  } catch (error) {
    toast.add({ severity: 'error', summary: 'Erro ao Atualizar Stock', detail: error, group: 'br', life: 3000 });
  }
}
...
async function onRowStateProgressUpdate(dataproduct) {
  ...
  try {
    ...
    if (index !== -1) {
      products.value[index].state_progress = response.state_progress;
      products.value[index].atribuicao = response.atribuicao;
      toast.add({ severity: 'success', summary: 'Progresso Atualizado', detail: `O Progresso foi atualizado para ${getNameStatus(response.state_progress, 1)}`, group: 'br', life: 3000 });
    }
  } catch (error) {
    toast.add({ severity: 'error', summary: 'Erro ao Atualizar Stock', detail: error, group: 'br', life: 3000 });
  }
...
const onCellEditComplete = async (event) => {
  ...
  if (field == 'comment') {
    ...
    if (index !== -1) {
      ...
      toast.add({ severity: 'success', summary: 'Comentario', detail: 'seu comentario foi adicionado ao item', group: 'br', life: 3000 });
    }
  }
};
</script>
...
.icon-container { display: flex !important; justify-content: center !important; align-items: center !important; height: 100% !important; }
.icon_valid { color: rgb(180, 180, 180) !important; }
.icon_invalid { color: rgb(255, 55, 55) !important; background-color: rgba(255, 139, 139, 0.141) !important; }
.data-table-container { height: calc(50vh - 150px); }
</style>

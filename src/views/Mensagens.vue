    ...
    <div class="flex gap-2">
      <div class="w-1/5" style="margin-left: 10px">
        <Menu
          :model="items"
          class="w-full"
          style="border-radius: 20px; height: calc(100vh - 80px)"
        >
          <template #start>
            <div class="flex justify-between" style="padding: 16px 20px 10px">
              <h3 style="margin: auto 0px">Mensagens</h3>
              <Button
                icon="pi pi-plus"
                severity="contrast"
                size="small"
          ...
          <template #item="{ item, props }">
            ...
            >
              <a
                ...
                v-bind="props.action"
                @click="navigate"
                :class="{ 'bg-emphasis rounded-border': isActive }"
              >
                <span
...
const notifications = ref({
  leroy: { notific: 0, alerts: 0, urgente: 0, saved: 0 },
  worten: { notific: 0, alerts: 0, urgente: 0, saved: 0 },
  total: 0,
  saved: 0,
});
...
const updateMenuBadges = () => {
  ...
  if (leroyGroup) {
    ...
    leroyGroup.badge =
      notifications.value.leroy.notific > 0
        ? notifications.value.leroy.notific
        : 0;

    // Badges dos subitens - apenas mostrar valores positivos
    updateItemBadgeIfPositive(
      leroyGroup,
      "unread_leroy",
      notifications.value.leroy.notific
    );

    // Adicionar badge para alertas Leroy
    updateItemBadgeIfPositive(
      leroyGroup,
      "alert_leroy",
      notifications.value.leroy.alerts
    );

    // Adicionar badge para salvos Leroy
    updateItemBadgeIfPositive(
      leroyGroup,
      "saved_leroy",
      notifications.value.leroy.saved || 0
    );
  }
  ...
  if (wortenGroup) {
    ...
    wortenGroup.badge =
      notifications.value.worten.notific > 0
        ? notifications.value.worten.notific
        : 0;
    ...
    // Adicionar badge para alertas Worten
    updateItemBadgeIfPositive(
      wortenGroup,
      "alert_worten",
      notifications.value.worten.alerts
    );

    // Adicionar badge para salvos Worten
    updateItemBadgeIfPositive(
      wortenGroup,
      "saved_worten",
      notifications.value.worten.saved || 0
    );
  }
  ...
  if (savedItem) {
    savedItem.badge =
      notifications.value.saved > 0 ? notifications.value.saved : 0;
  }
};
...
watch(
  [() => endpoint.params.marketplace, () => endpoint.params.state],
